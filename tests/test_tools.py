...
def _workspace(root: Path, mode: PermissionMode = PermissionMode.TRUSTED) -> WorkspacePolicy:
    return WorkspacePolicy(root, mode=mode)


def test_write_then_read(tmp_path: Path):
    f = tmp_path / "a.txt"
    workspace = _workspace(tmp_path)
    out = WriteFileTool(workspace).run({"path": str(f), "content": "hello\nworld"})
    assert "创建" in out
    read = ReadFileTool(workspace).run({"path": str(f)})
...
def test_read_offset_limit(tmp_path: Path):
    ...
    out = ReadFileTool(_workspace(tmp_path)).run({"path": str(f), "offset": 3, "limit": 2})
    assert "line3" in out and "line4" in out
    assert "line1" not in out and "line5" not in out


def test_read_missing(tmp_path: Path):
    out = ReadFileTool(_workspace(tmp_path)).run({"path": str(tmp_path / "nope.txt")})
    assert "不存在" in out
...
def test_edit_unique_match(tmp_path: Path):
    f = tmp_path / "c.py"
    f.write_text("x = 1\ny = 2\n")
    out = EditFileTool(_workspace(tmp_path)).run(
...
def test_edit_not_found(tmp_path: Path):
    ...
    f.write_text("hello\n")
    out = EditFileTool(_workspace(tmp_path)).run(
        {"path": str(f), "old_string": "zzz", "new_string": "b"}
    )
    assert "未找到" in out
...
def test_glob(tmp_path: Path):
    ...
    sub.mkdir()
    (sub / "z.py").write_text("")
    out = GlobTool(_workspace(tmp_path)).run({"pattern": "**/*.py", "path": str(tmp_path)})
    assert "x.py" in out and "z.py" in out
    assert "y.txt" not in out


def test_grep(tmp_path: Path, monkeypatch):
    (tmp_path / "g.py").write_text("def foo():\n    return 1\ndef bar():\n    pass\n")
    monkeypatch.setattr("minicoder.tools.grep.shutil.which", lambda _name: None)
    out = GrepTool(_workspace(tmp_path)).run({"pattern": r"def \w+", "path": str(tmp_path)})
...
def test_bash_basic(tmp_path: Path):
    out = BashTool(_workspace(tmp_path)).run({"command": "echo hello"})
    assert "hello" in out


def test_bash_dangerous_blocked(tmp_path: Path):
    ...
    out = BashTool(_workspace(tmp_path)).run({"command": "rm -rf /"})
    assert "已拒绝" in out
