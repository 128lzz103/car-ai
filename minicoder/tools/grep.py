...
class GrepTool(WorkspaceTool):
    ...
    def run(self, args: dict[str, Any]) -> str:
        ...
        try:
            ...
            if file_glob:
                self.workspace.validate_glob(file_glob)
        ...
        if shutil.which("rg"):
            return self._run_rg(pattern, str(root), file_glob)
    ...
    def _run_python(self, pattern: str, root: Path, file_glob: str | None) -> str:
        try:
        ...
        if root.is_file():
            ...
            paths = root.rglob(file_glob) if file_glob else root.rglob("*")
        for p in paths:
            ...
            for i, line in enumerate(text.splitlines(), 1):
                if regex.search(line):
                    results.append(f"{safe_path}:{i}:{line}")
