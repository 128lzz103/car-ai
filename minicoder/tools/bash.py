...
from ..security import RiskLevel, WorkspacePolicy
...
_DANGEROUS_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    (
        re.compile(r"\brm\s+(-[a-zA-Z]*\s+)*(-[a-zA-Z]*r[a-zA-Z]*\s+)*.*\s+/(\s|$)"),
        "递归删除根目录",
    ),
    (
...
_HIGH_RISK_PATTERN = re.compile(
    r"\b(rm|del|erase|rmdir|mv|move|chmod|chown|git\s+(reset|clean)|"
    r"pip\s+install|npm\s+(install|publish))\b",
    re.IGNORECASE,
)
_LOW_RISK_PATTERN = re.compile(
    r"^\s*(pwd|ls|dir|cat|type|head|tail|rg|grep|find|git\s+(status|diff|log|show))\b",
    re.IGNORECASE,
)
...
def classify_shell_risk(command: str) -> RiskLevel:
    ...
    if _HIGH_RISK_PATTERN.search(command):
        return RiskLevel.HIGH
    if _LOW_RISK_PATTERN.search(command):
...
class BashTool(Tool):
    ...
    def __init__(self, workspace: WorkspacePolicy | None = None) -> None:
        self.workspace = workspace or WorkspacePolicy(Path.cwd())
    ...
    def run(self, args: dict[str, Any]) -> str:
        ...
        decision = self.workspace.authorize_shell(command, risk)
        if not decision.allowed:
            return f"已拒绝执行:{decision.reason}。"
